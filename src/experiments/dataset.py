...
## -------------------------------------------------------------------------------------------------
    def setup_spaces(self):
        ...
        self._state_space.add_dim(Dimension('sin th1'))
        self._state_space.add_dim(Dimension('cos th1'))
        self._state_space.add_dim(Dimension('sin th2'))
        self._state_space.add_dim(Dimension('cos th2'))
        dims = self._state_space.get_dim_ids()

        self._state_space = self._state_space.spawn([dims[4], dims[5], dims[1], dims[6], dims[7], dims[3]])
...
## -------------------------------------------------------------------------------------------------
    def _setup_dataset(self,
        ...
        # Create ip_values df, with state and action
        ip_values = pd.concat([self._states, self._actions], axis=1, copy=True).iloc[:-1].reset_index(drop=True)
        # Drop rows at episode change
        ip_values = ip_values.drop(labels=episode_idx[1:])
        # ip_values transform
        ip_values['cos_th1'] = ip_values['th1'].apply(lambda x: np.cos(np.radians(x)))
        ip_values['sin_th1'] = ip_values['th1'].apply(lambda x : np.sin(np.radians(x)))
        ip_values['cos_th2'] = ip_values['th2'].apply(lambda x: np.cos(np.radians(x)))
        ip_values['sin_th2'] = ip_values['th2'].apply(lambda x : np.sin(np.radians(x)))
        ip_values = ip_values.drop(columns = ['th1', 'th2'], axis = 0)
        ip_values = ip_values.iloc[:, [4,3,0,6,5,1,2]]
        ip_values = ip_values.values
        # Create op_values df
        op_values = self._states.iloc[1:].reset_index(drop=True)
        # Drop rows at episode change
        op_values = op_values.drop(labels=episode_idx[1:])
        op_values['cos_th1'] = op_values['th1'].apply(lambda x: np.cos(np.radians(x)))
        op_values['sin_th1'] = op_values['th1'].apply(lambda x: np.sin(np.radians(x)))
        op_values['cos_th2'] = op_values['th2'].apply(lambda x: np.cos(np.radians(x)))
        op_values['sin_th2'] = op_values['th2'].apply(lambda x: np.sin(np.radians(x)))
        op_values = op_values.drop(columns = ['th1', 'th2'], axis = 0)
        op_values = op_values.iloc[:, [3, 2, 0, 5, 4, 1]]
        op_values = op_values.values
        return ip_values, op_values
    ...
    def get_data(self, p_index):
        ...
        if self._op_state_indexes:
            ...
            if self._normalize:
                features = self._normalizer_feature_data.normalize(p_data = features)
                labels = self._normalizer_label_data.normalize(p_data = labels)
            labels = np.array([labels[i] for i in self._op_state_indexes])
            ...
            return Dataset.get_data(self, p_index= p_index)